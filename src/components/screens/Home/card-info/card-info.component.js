...
export class CardInfo extends ChildComponent {
	...
	fillElements() {
		...
		$Q(this.element).find('#card-expire-date').text(this.card.expireDate)

		const cardCvcElement = $Q(this.element).find('#card-cvc')
	...
	render() {
		if (this.store.state.user) {
			$Q(this.element).html(new Loader().render().outerHTML)
