...
export class TransferField extends ChildComponent {
    ...
    handleTransfer = (event) => {
        ...
        $Q(event.target).text('Sending...').attr('disabled', true)

        const inputElement = $Q(this.element).find('input')
        const toCardNumber = inputElement.value().replaceAll('-', '')

        const reset = () => {
            $Q(event.target).removeAttr('disabled').text('Send')
        }

        if (!toCardNumber) {
            validationService.showError($Q(this.element).find('label'))
            reset()
            return
        }
