...
export class Statistics extends ChildComponent {
	...
	fetchData() {
		this.statisticService.main(data => {
			...
			const statisticsItemsElement = $Q(this.element).find('#statistics-items')
			statisticsItemsElement.text('')

			const circleChartElement = $Q(this.element).find('#circle-chart')
	...
	render() {
		if (this.store.user) {
			$Q(this.element).append(new Loader().render())
